...
*** NOT FUNCTIONAL ***
Program that generates an equivalent OBJ file (using cones, not cylinders like the original)
Useful information may be found here to transform the cylinder into cones: https://www.cuemath.com/geometry/frustum-of-cone/
# Retrieved from https://github.com/Robbie1977/NRRDtools/blob/master/swc_to_obj.py
# By Robbie1977
...
def create_mesh_from_swc(swc_data, minRadius=0.005):
    ...
    for i, node in enumerate(swc_data):
        ...
        # Create a cone (cylinder) for each edge
        if node['parent'] != -1:
            parent_node = swc_data[np.where(swc_data['id'] == node['parent'])[0][0]]

            # Calculate cone (cylinder) properties
            ...
            if length > 0:  # Check if the length is greater than zero before proceeding
                ...
                cone = trimesh.creation.cone(radius=max(radius, minRadius), height=length, sections=16)
...
# # Example usage
swc_file = 'retinal_cells/output1.swc'
obj_file = 'retinal_cells_meshified/output1.obj'
